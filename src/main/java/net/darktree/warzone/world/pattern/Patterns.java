...
public class Patterns {
	...
	public static final Pattern STAR_SMALL = FixedPattern.create().add(-1, -1).add(-1, 1).add(1, 1).add(1, -1).build();
	public static final Pattern STAR_LARGE = FixedPattern.create().add(2, 0).add(-2, 0).add(0, 2).add(0, -2).build();

	public static Pattern nextColonizationPattern(int dice) {
		return dice == 2 ? LARGE_CROSS : SMALL_CROSS;
	}
